...
#ifndef SKIMMERSENSE_ZIGBEE_SETTLE_MS
#define SKIMMERSENSE_ZIGBEE_SETTLE_MS 2500UL
#endif

#ifndef SKIMMERSENSE_REPORT_SETTLE_MS
#define SKIMMERSENSE_REPORT_SETTLE_MS 1200UL
#endif

#include "Zigbee.h"

static constexpr char FIRMWARE_VERSION[] = "0.9-deepsleep-zigbee-stage1";
...
// Preserve the already-interviewed endpoint layout.
...
void configureZigbeeEndpoints() {
  ...
  if (!zbTemperature.setPowerSource(
    ...
    Serial.println("Power Configuration cluster setup failed");
  }
...
bool startZigbee() {
  ...
  if (!Zigbee.begin(&zigbeeConfig, false)) {
    Serial.println("Zigbee begin failed; sleep will continue without a reboot loop.");
    return false;
  }
  ...
  if (!Zigbee.connected()) {
    Serial.println("Zigbee reconnect timeout; retry on next wake.");
    return false;
  }
...
void updateZigbeeAttributes(bool lowRaw, bool highRaw, float temperatureC) {
  // Attribute updates are intentionally separated from explicit reports.
  // The continuously-awake firmware proved these local updates are safe.
  ...
  if (readMax17048(battery)) {
    ...
    Serial.printf("Battery attributes: %u %% | %.3f V | raw SOC %.1f %%\n",
                  percentage, battery.voltage, battery.soc);
  }
}

bool sendStage1TemperatureReport(float temperatureC) {
  if (temperatureC == DEVICE_DISCONNECTED_C || !Zigbee.connected()) {
    return false;
  }

  // The previous sleepy build crashed when the first explicit Binary Input
  // report was sent immediately after reconnect. Stage 1 deliberately avoids
  // ALL explicit binary/battery reports and tests only the temperature report,
  // after a generous post-connect settle delay.
  Serial.printf("Zigbee stage1: settling %lu ms before explicit report...\n",
                static_cast<unsigned long>(SKIMMERSENSE_ZIGBEE_SETTLE_MS));
  delay(SKIMMERSENSE_ZIGBEE_SETTLE_MS);

  Serial.println("Zigbee stage1: reportTemperature() -> start");
  const bool ok = zbTemperature.reportTemperature();
  Serial.printf("Zigbee stage1: reportTemperature() -> %s\n", ok ? "OK" : "FAILED");
  return ok;
}
...
void prepareAndEnterDeepSleep(bool lowRaw, bool highRaw, bool intRaw) {
  // Let the one explicit temperature report leave the radio queue.
  delay(SKIMMERSENSE_REPORT_SETTLE_MS);

  // Re-read switch states after Zigbee activity so a movement during the awake
  // window cannot arm the wrong wake polarity.
  lowRaw = digitalRead(PIN_FLOAT_LOW);
  highRaw = digitalRead(PIN_FLOAT_HIGH);
  intRaw = digitalRead(PIN_MAX17048_INT);
...
void setup() {
  ...
  Serial.println(" Deep-sleep + Zigbee staged validation");
  Serial.println(" Stage 1: temperature report only");
  Serial.println(" Binary and battery explicit reports DISABLED");
  Serial.println("========================================");
  printWakeReason();
  ...
  if (connected) {
    Serial.println("Zigbee stage1: updating local endpoint attributes...");
    updateZigbeeAttributes(lowRaw, highRaw, temperatureC);
    Serial.println("Zigbee stage1: local attributes updated.");
    sendStage1TemperatureReport(temperatureC);
    Serial.println("Zigbee stage1: binary explicit reports remain disabled.");
    Serial.println("Zigbee stage1: battery explicit report remains disabled.");
  }

  prepareAndEnterDeepSleep(lowRaw, highRaw, intRaw);
}
